    ...
    def from_dict(cls, config_dict: dict[str, Any]) -> SimulationConfig:
        ...
        if "algorithm" in data and isinstance(data["algorithm"], str):
            try:
                ...
                raise ConfigurationError(
                    f"Unknown algorithm: {data['algorithm']}", config_key="algorithm"
                )
        ...
        if "output_format" in data and isinstance(data["output_format"], str):
            try:
                ...
                raise ConfigurationError(
                    f"Unknown output format: {data['output_format']}", config_key="output_format"
                )
        ...
        if "log_level" in data and isinstance(data["log_level"], str):
            try:
                ...
                raise ConfigurationError(
                    f"Unknown log level: {data['log_level']}", config_key="log_level"
                )
        ...
        if unknown_keys:
            raise ConfigurationError(
                f"Unknown configuration keys: {unknown_keys}", config_key=list(unknown_keys)[0]
            )

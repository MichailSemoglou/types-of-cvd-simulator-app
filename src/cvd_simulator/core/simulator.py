...
class CVDSimulator:
    ...
    def simulate(self, image: Image.Image, cvd_type: CVDType) -> Image.Image:
        ...
        try:
            ...
            raise CVDSimulatorError(
                f"Failed to simulate {cvd_type.name}: {e}"
            )
...
class AsyncCVDSimulator(CVDSimulator):
    ...
    def process_batch_parallel(
        ...
        logger.info(f"Starting parallel batch processing: {total} images, "
        ...
        if cvd_type:
            ...
            args_list = [
                (str(path), cvd_type.name, config_dict)
                for path in image_paths
            ]
        ...
        with ProcessPoolExecutor(max_workers=self.max_workers) as executor:
            futures = {
                executor.submit(process_func, args): Path(args[0])
        ...
        if failed:
            logger.warning(
                f"Batch complete: {success_count} succeeded, {len(failed)} failed"
            )
            if success_count == 0:
                raise BatchProcessingError(
                    "All images failed to process",
    ...
    def process_batch_chunked(
        ...
        logger.info(f"Processing {total} images in {len(chunks)} chunks "
                   f"(size: {chunk_size})")

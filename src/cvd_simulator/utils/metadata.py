...
def create_metadata(
    ...
    output_dict = {cvd_type.name: str(path) for cvd_type, path in output_files.items()}
...
def export_metadata(metadata: SimulationMetadata, output_path: Path | str, indent: int = 2) -> Path:
...
def verify_reproducibility(
    metadata_path: Path | str, input_file: Path | str | None = None
) -> dict[str, Any]:
    ...
    if metadata.version == __version__:
        ...
        results["warnings"].append(
            f"Software version differs: metadata={metadata.version}, " f"current={__version__}"
        )

    # Check system compatibility
    current_system = get_system_info()
    if metadata.system_info.get("platform") != current_system["platform"]:
        results["warnings"].append("Platform differs, results may vary slightly")

    return results


def generate_sidecar_path(image_path: Path | str, suffix: str = "_metadata") -> Path:
